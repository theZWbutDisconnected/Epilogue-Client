...
public abstract class Shader {
    ...
    private void createProgram(String vertexShader, String fragmentShader) {
        this.programId = GL20.glCreateProgram();
        GL20.glAttachShader(this.programId, this.compileShader(vertexShader, GL20.GL_VERTEX_SHADER));
        GL20.glAttachShader(this.programId, this.compileShader(fragmentShader, GL20.GL_FRAGMENT_SHADER));
        GL20.glLinkProgram(this.programId);
        int programId = GL20.glGetProgrami(this.programId, GL20.GL_LINK_STATUS);
        if (programId == 0) {
            this.programId = -1;
