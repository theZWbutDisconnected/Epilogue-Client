...
public class TextField extends Gui {
    ...
    private float cursorVerticalOffset = -1.5f;
    private Color outline = Color.WHITE, fill = ColorUtil.applyOpacity3(32, 1);
    ...
    private Animation hoverAnimation = new DecelerateAnimation(250, 1);
    ...
    public void mouseClicked(int mouseX, int mouseY, int mouseButton) {
        ...
        if (this.isFocused && flag && mouseButton == 0) {
            ...
            this.setCursorPosition(FontRenderer.trimStringToWidth(s, (int) i).length() + this.lineScrollOffset);
        }
    }
    ...
    public void drawTextBox() {
        if (this.getVisible()) {
            if (this.getEnableBackgroundDrawing()) {
                drawRect((int) this.xPosition - 1, (int) this.yPosition - 1, (int) this.xPosition + (int) this.width + 1, (int) this.yPosition + (int) this.height + 1, -6250336);
                drawRect((int) this.xPosition, (int) this.yPosition, (int) this.xPosition + (int) this.width, (int) this.yPosition + (int) this.height, -16777216);
            }

            int i = this.isEnabled ? this.focusedTextColor.getRGB() : this.unfocusedTextColor.getRGB();
            ...
            if (!s.isEmpty()) {
                ...
                FontRenderer.drawStringWithShadow(s1, (float) l, (float) i1, i);
                j1 = l + FontRenderer.getStringWidth(s1);
            }
            ...
            if (!s.isEmpty() && flag && j < s.length()) {
                ...
                j1 = j1 + FontRenderer.getStringWidth(s.substring(j));
            }

            if (flag1) {
                if (flag2) {
                    Gui.drawRect(k1, i1 - 1, k1 + 1, i1 + 1 + FontRenderer.getFontHeight(), -3092272);
                } else {
            ...
            if (k != j) {
                ...
                this.drawCursorVertical(k1, i1 - 1, l1 - 1, i1 + 1 + FontRenderer.getFontHeight());
            }
        }
    }
    ...
    private void drawCursorVertical(int startX, int startY, int endX, int endY) {
        ...
        if (endX > this.xPosition + this.width) {
            endX = (int) this.xPosition + (int) this.width;
        }

        if (startX > this.xPosition + this.width) {
            startX = (int) this.xPosition + (int) this.width;
        }
    ...
    public void setTextColor(int color) {
        this.focusedTextColor = new Color(color);
    }

    public void setDisabledTextColour(int color) {
        this.unfocusedTextColor = new Color(color);
    }
    ...
    public int getWidth() {
        return this.getEnableBackgroundDrawing() ? (int) this.width - 8 : (int) this.width;
    }
    ...
    public void setCursorVerticalOffset(float cursorVerticalOffset) {
        this.cursorVerticalOffset = cursorVerticalOffset;
    }

    public void setSelectionBoxVerticalOffset(float selectionBoxVerticalOffset) {
    }
